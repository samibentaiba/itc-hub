  ...
  return (
    <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
      {departments.map((department) => {
        // Map members by role
        const leaders = department.members.filter((m: any) => m.role === "LEADER" || m.role === "SUPERLEADER").map((m: any) => m.user)
        const members = department.members.filter((m: any) => m.role === "MEMBER").map((m: any) => m.user)

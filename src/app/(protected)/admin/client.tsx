...
export default function AdminClientPage({ initialUsers, initialTeams, initialDepartments }: AdminClientPageProps) {
  ...
  } = useAdminPage(initialUsers, initialTeams, initialDepartments);
  ...
  const isDeptFormLoading =
    ...
    (modal?.view === 'EDIT_DEPARTMENT' && loadingAction === `edit-${modal.data.id}`);
  ...
  const entityForDialog = currentManagingEntity
    ? {
        ...
        entityType: modal?.view === 'MANAGE_MEMBERS' ? modal.data.entityType : 'team',
      }
    : null;
  ...
  return (
    <>
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div>
            <h1 className="text-xl sm:text-2xl font-bold flex items-center gap-2">
              <Shield className="h-5 w-5 sm:h-6 sm:w-6 text-red-500" />Admin Panel
            </h1>
            <p className="text-sm text-muted-foreground">Manage users, teams, and departments</p>
          </div>
          <div className="flex gap-2">
            <Button variant="outline" size="sm" onClick={handleRefreshData} disabled={loadingAction === "refresh"}>
              <RefreshCw className={`mr-2 h-4 w-4 ${loadingAction === 'refresh' ? 'animate-spin' : ''}`} />Refresh
            </Button>
            <Button variant="outline" size="sm" onClick={handleExportData} disabled={loadingAction === "export"}>
              <Download className={`mr-2 h-4 w-4 ${loadingAction === 'export' ? 'animate-spin' : ''}`} />Export
            </Button>
          </div>
        </div>
      ...
      <DepartmentFormDialog
        isOpen={modal?.view === 'ADD_DEPARTMENT' || modal?.view === 'EDIT_DEPARTMENT'}
        onClose={closeModal}
      ...
      <ActionConfirmationDialog
        isOpen={['DELETE_USER', 'VERIFY_USER', 'DELETE_TEAM', 'DELETE_DEPARTMENT'].includes(modal?.view || '')}
        onClose={closeModal}
        onConfirm={handleActionConfirm}
        isLoading={!!loadingAction}
        title={
            modal?.view === 'VERIFY_USER' ? 'Confirm Verification' : 'Are you absolutely sure?'
        }
        description={
            modal?.view === 'DELETE_USER' ? `This will permanently delete ${modal.data.name}.` :
            modal?.view === 'VERIFY_USER' ? `This will mark ${modal.data.name} as verified.` :
            modal?.view === 'DELETE_TEAM' ? `This will permanently delete the ${modal.data.name} team.` :
            modal?.view === 'DELETE_DEPARTMENT' ? `This will permanently delete the ${modal.data.name} department and all its teams.` :

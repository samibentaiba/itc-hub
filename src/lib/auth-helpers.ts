...
export async function isTicketCreator(userId: string, ticketId: string): Promise<boolean> {
...
export async function canManageDepartment(userId: string, departmentId: string): Promise<boolean> {
  return (await isAdmin(userId)) || (await isDepartmentManager(userId, departmentId));
}

export async function canManageTicket(userId: string, ticketId: string): Promise<boolean> {
  if (await isAdmin(userId)) {
    return true;
  }
  ...
  if (ticket?.departmentId && await isDepartmentManager(userId, ticket.departmentId)) {
    return true;
  }
